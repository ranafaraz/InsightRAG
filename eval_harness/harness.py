...
def render_markdown(settings: Settings) -> str:
    ...
    lines.append("")
    lines.append(
        "> Generated by `python -m eval_harness.harness`. "
        "Numbers above are from the deterministic offline backends; switching to "
        "`sentence-transformers` + `cross-encoder` raises retrieval and answer quality "
        "further (re-run with those env vars to regenerate)."
    )
    return "\n".join(lines)
